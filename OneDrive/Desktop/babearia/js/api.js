(() => {
  const defaultConfig = {
    ...
    payloads: {
      ...
      passwordResetConfirm: ({ email, code, newPassword }) => ({
        email,
        code,
        newPassword
      }),
      ...
      meUpdate: ({ name, telefone, password }) => ({
        name,
        telefone,
        password
      })
  ...
  const config = mergeConfig(window.BARBERIA_API_CONFIG || {});
  ...
  const normalizeService = (service = {}) => {
    ...
    return {
      ...
      name,
      description: service.descricao || service.description || "Servico premium Dom Pedro.",
      price,
      status,
  ...
  const extractUser = (data, fallbackEmail) =>
    data?.user || data?.usuario || data?.cliente || (fallbackEmail ? { email: fallbackEmail } : null);
  ...
  const login = async (credentials) => {
    ...
    try {
      ...
      if (credentials?.login) {
        setUser({ username: credentials.login });
      }
    }
    return data;
  };
  ...
  const getServices = () =>
    apiRequest(config.endpoints.services, { method: "GET", auth: false });
  ...
  const getMyAppointments = () =>
    apiRequest(config.endpoints.myAppointments, { method: "GET" });
  ...
  const getBarberAvailability = (filters = {}) => {
    const params = new URLSearchParams(filters);
    const query = params.toString();
    return apiRequest(`${config.endpoints.appointmentAvailability}${query ? `?${query}` : ""}`, {
      method: "GET"
    });
  };
  ...
  const deleteService = (id) =>
    apiRequest(`${config.endpoints.services}/${id}`, { method: "DELETE" });
